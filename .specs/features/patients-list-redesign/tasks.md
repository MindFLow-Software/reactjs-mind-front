...
- [ ] Envolver o componente em `React.memo`
- [ ] Converter handlers de modal para `useCallback` (`handleOpenDetails`, `handleOpenEdit`, `handleOpenDelete`)
- [ ] Remover colunas: CPF, Gênero, Idade (total: 9 → 6 colunas)
- [ ] Nova célula Avatar+Nome: `UserAvatar` com `size="md"` + nome completo em `font-medium`
- [ ] Badge de status: Ativo = `bg-emerald-500/15 text-emerald-600`; Inativo = `bg-muted text-muted-foreground`
- [ ] Nova célula "Última Sessão":
  - Valor: `formatDistanceToNow(new Date(lastSessionAt), { addSuffix: true, locale: ptBR })`
  - Null: `<span className="text-muted-foreground text-sm">Sem sessões</span>`
  - Envolver em `<Tooltip>` com data exata no `TooltipContent`
- [ ] Substituir 3 botões soltos por `DropdownMenu` com trigger `MoreHorizontal`
  - `aria-label={`Ações do paciente ${patient.name}`}`
  - Item "Ver detalhes" → `setDetailsOpen(true)`
  - Item "Editar" → `setEditOpen(true)`
  - `DropdownMenuSeparator`
  - Item "Inativar/Reativar" → `setDeleteOpen(true)` com `className="text-destructive focus:text-destructive"`
- [ ] `hover:bg-muted/50 transition-colors` na `<TableRow>`
