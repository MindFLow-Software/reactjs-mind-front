  ...
  const SUB_CARD_CONTENT_LIST = [
    {
      ...
      list: ['Ansiedade generalizada', 'Autoexigência profissional', 'Padrão de sono irregular'],
    },
    {
      id: useId(),
      icon: Award,
    ...
    },
    {
      ...
      list: ['Sobrecarga de trabalho', 'Isolamento pontual', 'Padrões perfeccionistas'],
    },
  ]
    ...
    <Card className="sr-card">
      <CardHeader className="sr-header">
        <div>
          ...
          <CardDescription className="text-[13px]">Gerado a partir das últimas 6 sessões</CardDescription>
        </div>
            ...
            return (
              <Card key={id} className="sr-sub-card">
                <CardHeader className="sr-sub-card-header">
                  <div className="sr-header-title-row">
                    <div className={cn('sr-sub-card-icon-wrap', `sr-icon-wrap-${color}`)}>
                ...
                <CardContent className="sr-sub-card-content">
                  <ul className="sr-sub-card-list">
                    {list.map((item, i) => (
                      <li key={i} className="sr-sub-card-list-item">{item}</li>
                    ))}
                  </ul>
                </CardContent>
              </Card>
