...
function TableBodyContent({ isLoading, patients, perPage, colSpan, hasActiveFilters, onClearFilters, onRegister }: TableBodyContentProps) {
    if (isLoading) return <PatientsTableLoading rows={perPage} />
    if (patients.length > 0) return <>{patients.map((patient) => <PatientsTableRow key={patient.id} patient={patient} />)}</>
    return (
        <TableRow>
            ...
            <Table>
                <TableHeader>
                    <TableRow className="bg-muted/30 hover:bg-muted/30">
                        ...
                        <SortableHead column="name"        label="Paciente"      sort={sort} className="min-w-[180px]" />
                        <SortableHead column="status"      label="Status"        sort={sort} className="w-[120px]" />
                        <SortableHead column="contact"     label="Contato"       sort={sort} className="min-w-[200px]" />
                        <SortableHead column="lastSession" label="Última Sessão" sort={sort} className="w-[140px] hidden lg:table-cell" />
                        <SortableHead column="age"         label="Idade"         sort={sort} className="w-[110px] hidden xl:table-cell" />
                        <SortableHead column="gender"      label="Gênero"        sort={sort} className="w-[110px] hidden xl:table-cell" />
