...
function AttachmentsBody({ isLoading, attachments, isRemoving, onRemove }: AttachmentsBodyProps) {
    if (isLoading) return (
    ...
    if (!attachments || attachments.length === 0) return (
        <p className="rp-att-empty">Nenhum documento enviado ainda.</p>
    )
    return (
        <div className="rp-att-grid">
            {attachments.map((file) => {
                ...
                const uploadDate = format(parseISO(file.uploadedAt), "dd/MM/yyyy", { locale: ptBR })
                return (
