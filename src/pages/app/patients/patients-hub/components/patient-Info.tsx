...
import { Copy, SquarePen, MapPin, Activity, User, Video, Plus } from "lucide-react"
...
function GenderBadge({ gender }: { gender: Gender }) {
    const map: Record<Gender, { label: string; symbol: string; className: string }> = {
        MASCULINE: { label: "Masculino", symbol: "♂", className: "bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300" },
        FEMININE:  { label: "Feminino",  symbol: "♀", className: "bg-pink-50 text-pink-700 dark:bg-pink-950/40 dark:text-pink-300" },
        OTHER:     { label: "Outro",     symbol: "◈", className: "bg-purple-50 text-purple-700 dark:bg-purple-950/40 dark:text-purple-300" },
    }
...
function AddLink({ label }: { label: string }) {
    return (
        <button type="button" className="text-sm text-blue-600 dark:text-blue-400 font-medium hover:underline cursor-pointer">
            + {label}
        </button>
    )
}
...
function FieldRow({ label, value, last }: { label: string; value: React.ReactNode; last?: boolean }) {
    return (
        <div className={cn("flex items-center gap-4 py-3", !last && "border-b border-border/40")}>
        ...
        <div className="space-y-4 mt-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                >
                    ...
                    <FieldRow label="Indicação"      value={<span className="text-muted-foreground">{DASH}</span>} />
            ...
            >
                <div className="flex items-center justify-center py-6 text-sm text-muted-foreground">
                    ...
                    <button type="button" className="text-blue-600 dark:text-blue-400 font-medium hover:underline cursor-pointer">
                        Adicionar agora
                    </button>
                </div>
            </CardSection>
