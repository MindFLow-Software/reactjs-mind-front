...
export function ApprovalsDetailsDialog({
  psychologist,
  onApprove,
  onClose,
}: ApprovalsDetailsDialogProps) {
  return (
    <DialogContent className="max-w-md p-0 overflow-hidden flex flex-col gap-0">
      <DialogHeader className="p-6 pb-4 border-b">
        <DialogTitle className="flex items-center gap-2 text-base">
          <BadgeCheck className="h-5 w-5 text-primary" /> Verificação
          Profissional
        </DialogTitle>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              {formatCPF(psychologist.cpf)}
            </div>
            <div className="flex items-center gap-2 text-sm">
              <Phone className="h-4 w-4 text-muted-foreground/60" />
              <span className="font-medium text-muted-foreground">Número:</span>
              <a
                href={`https://wa.me/55${psychologist.phoneNumber}`}
                target="_blank"
      ...
      <div className="p-4 bg-muted/30 border-t flex justify-end gap-2">
        <Button
          ...
          size="sm"
          onClick={onClose}
          className="cursor-pointer h-8 text-xs px-4"
        >
          Fechar
        </Button>
        <Button
          size="sm"
          className="cursor-pointer h-8 text-xs bg-green-600 hover:bg-green-700 px-4"
          onClick={() => onApprove(psychologist.id)}
        >
