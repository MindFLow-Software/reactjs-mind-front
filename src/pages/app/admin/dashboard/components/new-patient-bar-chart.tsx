...
import { getTotalPatientsAdminChart } from "@/api/get-total-patients-admin-chart"
        ...
        <Card className="col-span-full lg:col-span-6 border-border bg-card shadow-sm rounded-2xl overflow-hidden">
            <CardHeader className="flex flex-col items-stretch space-y-0 border-b border-border p-0 sm:flex-row">
                <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-4">
                    <CardTitle className="text-base font-semibold text-foreground">Fluxo de Pacientes</CardTitle>
                    <CardDescription className="text-sm text-muted-foreground">
                        Novos pacientes registrados na plataforma
                    </CardDescription>
                </div>
            ...
            <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6 bg-card">
                {isLoading ? (
                    ...
                    <div className="flex h-[250px] flex-col items-center justify-center text-center">
                        ...
                        <p className="text-sm font-medium text-foreground">Nenhum paciente</p>
                        <p className="mt-1 text-xs text-muted-foreground">Sem novos registros no intervalo selecionado</p>
                    </div>
                ) : (
                    <ChartContainer
                        config={chartConfig}
