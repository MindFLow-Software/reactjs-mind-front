...
import {
    Bell, Share2, X, Link2, MessageCircle,
    ChevronUp, Check, ChevronRight, Zap,
    Clock, Users, Paperclip, AtSign, Bold,
    TriangleAlert, Eye,
} from "lucide-react"
...
const CATEGORY_CONFIG: Record<CategoryKey, {
    ...
    UI_UX:        { label: "Fluxo",       dot: "bg-violet-500",  pillBg: "bg-violet-50 dark:bg-violet-950/30",  pillText: "text-violet-700 dark:text-violet-300"  },
    REPORTS:      { label: "Relatórios",  dot: "bg-amber-500",   pillBg: "bg-amber-50 dark:bg-amber-950/30",   pillText: "text-amber-700 dark:text-amber-300"   },
    INTEGRATIONS: { label: "Integrações", dot: "bg-blue-500",    pillBg: "bg-blue-50 dark:bg-blue-950/30",    pillText: "text-blue-700 dark:text-blue-300"    },
    SCHEDULING:   { label: "Paciente",    dot: "bg-pink-500",    pillBg: "bg-pink-50 dark:bg-pink-950/30",    pillText: "text-pink-700 dark:text-pink-300"    },
    PRIVACY_LGPD: { label: "Financeiro",  dot: "bg-emerald-500", pillBg: "bg-emerald-50 dark:bg-emerald-950/30", pillText: "text-emerald-700 dark:text-emerald-300" },
    OTHERS:       { label: "Outros",      dot: "bg-slate-400",   pillBg: "bg-slate-100 dark:bg-slate-800",    pillText: "text-slate-600 dark:text-slate-300"   },
}
...
const STATUS_CONFIG: Record<SuggestionStatus, {
    ...
    PENDING:      { label: "AGUARDANDO",    bannerBg: "bg-blue-50 dark:bg-blue-950/30",     bannerText: "text-blue-700 dark:text-blue-400",     blobColor: "bg-blue-500",    currentStep: 0 },
    OPEN:         { label: "EM VOTAÇÃO",    bannerBg: "bg-blue-50 dark:bg-blue-950/30",     bannerText: "text-blue-700 dark:text-blue-400",     blobColor: "bg-blue-500",    currentStep: 1 },
    UNDER_REVIEW: { label: "EM ESTUDO",     bannerBg: "bg-purple-50 dark:bg-purple-950/30", bannerText: "text-purple-700 dark:text-purple-400", blobColor: "bg-purple-500", currentStep: 2 },
    PLANNED:      { label: "IMPLEMENTANDO", bannerBg: "bg-amber-50 dark:bg-amber-950/30",   bannerText: "text-amber-700 dark:text-amber-400",   blobColor: "bg-amber-500",  currentStep: 3 },
    IMPLEMENTED:  { label: "CONCLUÍDO",     bannerBg: "bg-emerald-50 dark:bg-emerald-950/30", bannerText: "text-emerald-700 dark:text-emerald-400", blobColor: "bg-emerald-500", currentStep: 4 },
    REJECTED:     { label: "REJEITADA",     bannerBg: "bg-red-50 dark:bg-red-950/30",       bannerText: "text-red-700 dark:text-red-400",       blobColor: "bg-red-500",    currentStep: -1 },
}
...
const TIMELINE_CONFIG: Record<SuggestionStatus, { doneUntil: number; currentIdx: number }> = {
...
export function SuggestionDetailModalContent({
    ...
    const formattedDate = format(new Date(item.createdAt), "dd 'de' MMM. 'de' yyyy", { locale: ptBR })
    const relativeDate = formatDistanceToNow(new Date(item.createdAt), { addSuffix: true, locale: ptBR })
        ...
        <DialogPortal>
            ...
            >
                ...
                <div className={cn(
                    ...
                    <span className={cn("size-2 rounded-full shrink-0", statusCfg.blobColor)} />
                    <span className={cn("text-[11.5px] font-bold tracking-[0.07em] shrink-0", statusCfg.bannerText)}>
                        {statusCfg.label}
                    </span>
                    ...
                    {isImplemented ? (
                        <div className="flex flex-col items-center justify-center gap-1 shrink-0 w-16 py-2">
                            <div className="size-10 rounded-full bg-emerald-100 dark:bg-emerald-950/30 flex items-center justify-center">
                                <Check className="size-5 text-emerald-600 dark:text-emerald-400" strokeWidth={2.5} />
                            </div>
                        ...
                        >
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-[0.06em]">VOTOS</span>
                        </button>
                    )}
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[11.5px] text-muted-foreground">
                                    por <strong className="font-semibold text-foreground">{item.psychologistName}</strong>
                                </span>
                            </div>
                            <span className="size-[3px] rounded-full bg-slate-300 dark:bg-slate-600 shrink-0" />
                            <span className="text-[11.5px] text-muted-foreground" title={formattedDate}>
                                {relativeDate}
                            </span>
                            <span className="size-[3px] rounded-full bg-slate-300 dark:bg-slate-600 shrink-0" />
                                ...
                                >
                                    {item.attachments.map((att, i) => (
                                        <button
                                            key={i}
                                            type="button"
                                            onClick={() => setPreviewItem({ url: buildAttachmentUrl(att), name: att })}
                                        ...
                                        >
                                            ...
                                            <div className="absolute bottom-0 inset-x-0 h-8 bg-gradient-to-t from-black/70 to-transparent flex items-end px-2 pb-1">
                                                <span className="text-[10px] text-white font-medium truncate">{att}</span>
                                            </div>
                                        </button>
                                    ))}
                                </div>
                            ...
                            {item.likesCount > 0 ? (
                                <div className="space-y-2">
                                    <div className="flex items-center">
                                        {Array.from({ length: Math.min(5, item.likesCount) }).map((_, i) => (
                                            <div
                                                key={i}
                                                className={cn(
                                    ...
                                    <p className="text-[12.5px] text-muted-foreground">
                                        <strong className="text-foreground font-bold">{item.likesCount}</strong> psicólogos votaram
                                    </p>
                                </div>
                            ) : (
                                <p className="text-[12px] text-muted-foreground italic">Nenhum voto ainda.</p>
                            )}
                        </div>
                            ...
                            <div className="relative">
                                ...
                                <div className="space-y-0">
                                    {TIMELINE_STEPS.map((step, i) => {
                                        ...
                                        return (
                                            <div key={i} className="relative flex items-center gap-3 py-2 pl-8">
                                                <div className={cn(
                ...
                {previewItem && (
                    <div className="absolute inset-0 z-20 flex flex-col overflow-hidden rounded-2xl bg-[#0d1117]">
                        <div className="flex items-center gap-3 px-5 py-4 shrink-0">
                            <p className="flex-1 min-w-0 text-sm font-semibold text-white truncate" title={previewItem.name}>
                                {previewItem.name}
                            </p>
                            <button
                                type="button"
